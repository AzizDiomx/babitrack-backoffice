...
export default function SubscribersPage() {
  ...
  const getStatusBadgeStyle = (status: string) => {
    switch (status) {
      ...
      case 'EXPIRE':
        return 'bg-slate-700/20 text-zinc-400 border border-slate-700/35';
      default:
    ...
    <div className="min-h-full bg-black p-6 lg:p-8 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Controls Bar */}
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-[#121212] p-4 rounded-2xl border border-zinc-800">
            {/* Search */}
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <select
                ...
                className="w-full sm:w-auto bg-black border border-zinc-800 rounded-xl px-4 py-2 text-sm text-zinc-300 focus:outline-none focus:border-orange-500 transition"
              >
          ...
          <div className="overflow-hidden rounded-2xl border border-zinc-800 bg-[#121212] backdrop-blur-xl">
            {loading ? (
              ...
              <div className="text-center py-20 text-zinc-500">
                ...
                <p className="font-semibold text-zinc-400">Aucun abonné trouvé</p>
                <p className="text-xs text-zinc-500 mt-1">Essayez d'ajuster votre recherche ou filtre.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-850 text-left">
                  <thead className="bg-[#121212]/60 text-xs font-bold text-slate-450 uppercase tracking-wider">
                    <tr>
                        ...
                        <td className="whitespace-nowrap px-6 py-4.5 text-right text-sm">
                          <div className="flex justify-end gap-2">
                            ...
                            <button
                              ...
                              className="inline-flex items-center justify-center h-7.5 w-7.5 bg-slate-800 hover:bg-slate-700 text-zinc-300 border border-slate-700/50 rounded-lg transition duration-150 cursor-pointer"
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-zinc-800 bg-[#121212] p-6 backdrop-blur-xl">
            ...
            <div
              ...
              className={`flex flex-col items-center justify-center border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition duration-150 ${
                isDragging 
                  ...
                  : 'border-zinc-800 hover:border-slate-700 bg-black/20'
              }`}
            >
              <input
                type="file"
              ...
              {uploading ? (
                <>
                  ...
                  <p className="text-sm font-semibold text-zinc-300">Traitement du fichier...</p>
                </>
              ) : (
                <>
            ...
            {importReport && (
              <div className="bg-black/60 border border-zinc-800 rounded-xl p-5 mt-6 space-y-3">
                <h4 className="text-sm font-bold text-zinc-200">Rapport d'importation</h4>
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-3">
                    <span className="text-lg font-bold text-emerald-400">{importReport.success}</span>
                    <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mt-1">Importés</p>
                  </div>
                  <div className="bg-red-500/5 border border-red-500/10 rounded-xl p-3">
                    <span className="text-lg font-bold text-red-400">{importReport.errors}</span>
                    <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mt-1">Échecs</p>
                  </div>
                </div>
                
                {importReport.details.length > 0 && (
                  <div className="pt-2 border-t border-slate-900">
                    <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-2">Erreurs rencontrées :</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-[#121212] rounded-3xl border border-zinc-800 shadow-2xl p-6 relative">
            <button
              ...
              className="absolute right-4 top-4 text-zinc-500 hover:text-slate-350 cursor-pointer"
            >
              <X className="h-5 w-5" />
            </button>
            ...
            <p className="text-xs text-zinc-400 mb-6">
              Configurez le forfait de transport pour <span className="font-bold text-zinc-200">{selectedUser.prenom} {selectedUser.nom}</span>.
            </p>

            <form onSubmit={handleActivateSubmit} className="space-y-4">
              {/* Type */}
              <div>
                <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-2">Type d'abonnement</label>
                <select
                  value={subType}
              ...
              {/* Price */}
              <div>
                <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-2">Montant (FCFA)</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-black border border-zinc-800 rounded-xl pl-9 pr-4 py-2.5 text-sm text-zinc-200 focus:outline-none focus:border-orange-500"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-2">Date début</label>
                  <div className="relative">
                    <input
                      type="date"
                      value={dateDebut}
                ...
                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-2">Date fin</label>
                  <div className="relative">
                    <input
                      type="date"
                      value={dateFin}
